...
export default function GuidesScreen({ onBack, onNavigate }) {
  ...
  const categorias = [
    ...
    { id: 3, nombre: "Luz y Clima", desc: "Luz y temperatura.", icono: <Sun size={24} className="text-[#D97706]" /> },
    { id: 4, nombre: "Nutrientes", desc: "Guía sobre fertilizantes.", icono: <Wind size={24} className="text-[#65A30D]" /> },
    { id: 5, nombre: "Plagas", desc: "Identificar problemas.", icono: <Bug size={24} className="text-[#DC2626]" /> },
    { id: 6, nombre: "Herramientas", desc: "Resumen de esenciales.", icono: <Hammer size={24} className="text-[#4B5563]" /> },
  ];
    ...
    <div className="relative w-full h-screen max-w-[400px] mx-auto bg-white flex flex-col font-sans overflow-hidden shadow-2xl border-x border-gray-100">
      ...
      <div className="flex-1 overflow-y-auto pb-24 hide-scrollbar z-10 px-6 pt-6">
        ...
        <div className="relative mb-6">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500"><Search size={20} /></div>
            <input type="text" placeholder="Buscar temas específicos..." className="w-full bg-white/80 border border-[#d1fae5] text-gray-700 placeholder-gray-400 pl-12 pr-4 py-4 rounded-2xl outline-none focus:border-[#3A7D44] transition-all text-sm font-medium shadow-sm backdrop-blur-sm" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            {categorias.map((cat) => (
                <div 
                    key={cat.id}
                    // CORRECCIÓN AQUÍ: Lógica para ir a Riego (id 1) o Suelos (id 2)
                    onClick={() => {
                        if (cat.id === 1) onNavigate('riego-detail');
                        if (cat.id === 2) onNavigate('suelos-detail');
                        if (cat.id === 3) onNavigate('clima-detail');
                        if (cat.id === 4) onNavigate('nutrientes-detail');
                        if (cat.id === 5) onNavigate('plagas-detail');
                        if (cat.id === 6) onNavigate('herramientas-detail');
                    }}
